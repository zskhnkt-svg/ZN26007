...
int main(void)
{
  ...
  MX_RF_Init();
  /* USER CODE BEGIN 2 */
  /* Give the host time to enumerate the CDC port and let you open a terminal */
  HAL_Delay(3000);
  Dbg_Print("=== BOOT: peripherals initialized ===\r\n");

  LL_HSEM_1StepLock( HSEM, 5 );

	/* PA10 configured as push-pull output for the LED */
	__HAL_RCC_GPIOA_CLK_ENABLE();
	GPIO_InitTypeDef GPIO_InitStruct_LED = {0};
	GPIO_InitStruct_LED.Pin = GPIO_PIN_10;
...
/**
  * @brief  Printf-style debug output routed over USB CDC (Virtual COM Port).
  *         Used as the implementation behind APP_DBG_MSG (see app_conf.h).
